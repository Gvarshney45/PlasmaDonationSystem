...
const RequestForm = () => {
  ...
  const PostData = async (e) => {
    ...
    if (!handleValidation()) {
      ...
      const res = await fetch("/api/createrequester", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="Receiver-form container mt-4 border shadow-lg p-5 mb-5 bg-white rounded rounded">
      <h2 className="heading text-dark  ">Request Plasma</h2>
      <hr />
      ...
      <div class="form-group">
        <button type="submit" onClick={PostData} class="btn btn-primary btn-lg">
          Submit
        </button>
      </div>
    </div>
